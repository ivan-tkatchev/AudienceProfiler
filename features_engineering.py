...
def add_os_version_fetures(train_df, test_df):
    train_df[['os']] = train_df.os.str.lower()
    test_df[['os']] = test_df.os.str.lower()
    all_ios_versions = set(np.concatenate([train_df[train_df.os.str.lower() == 'ios'].osv.unique(),
                                           test_df[test_df.os.str.lower() == 'ios'].osv.unique()]))
    all_android_versions = set(np.concatenate([train_df[train_df.os.str.lower() == 'android'].osv.unique(),
                                               test_df[test_df.os.str.lower() == 'android'].osv.unique()]))
    and_v_to_num = android_versions_to_numerical(all_android_versions)
    ios_v_to_num = ios_versions_to_numerical(all_ios_versions)
    train_df = parse_version(train_df, and_v_to_num, ios_v_to_num)
    test_df = parse_version(test_df, and_v_to_num, ios_v_to_num)
